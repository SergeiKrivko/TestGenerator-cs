...
internal class DirectoryNode : Node
{
    ...
    public override void Update(int? recurseLevel = null)
    {
        ...
        foreach (var node in SubNodes.ToArray())
        {
            ...
            {
                node.Update(recurseLevel - 1);
            }
        }

        try
        {
            SubNodes.AddRange(Info.GetDirectories()
                .Where(d => SubNodes.SingleOrDefault(i => i.Path == d.FullName) == null)
                .Select(d => new DirectoryNode(d)));
            SubNodes.AddRange(Info.GetFiles().Where(d => SubNodes.SingleOrDefault(i => i.Path == d.FullName) == null)
                .Select(d => new FileNode(d)));
        }
