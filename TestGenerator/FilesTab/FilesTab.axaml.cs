...
public partial class FilesTab : SideTab
{
    ...
    public FilesTab()
    {
        ...
        Node.RegexIcons[new Regex(@"[/\\](\w*\.)?[Mm][Aa][Kk][Ee][Ff][Ii][Ll][Ee](\.\w+)?$")] = "M4.11381 4.45455H7.90643L11.9121 14.2273H12.0826L16.0882 4.45455H19.8809V19H16.8979V9.53267H16.7772L13.013 18.929H10.9817L7.21751 9.49716H7.09677V19H4.11381V4.45455Z";
    ...
    private void Control_OnContextRequested(object? sender, ContextRequestedEventArgs e)
    {
        ...
        foreach (var creator in FileCreators.OrderBy(c => c.Priority).Where(c => c.Enabled))
        {
            var menuItem = new MenuItem
                { Header = creator.Name, Icon = new PathIcon { Data = PathGeometry.Parse(creator.Icon ?? "") } };
            menuItem.Click += (o, args) => CreateFile(SelectedItem?.Path, creator);
