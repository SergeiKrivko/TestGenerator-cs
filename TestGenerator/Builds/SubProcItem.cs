...
public class SubProcItem : StackPanel
{
    ...
    public static readonly StyledProperty<BuildSubprocess?> SubprocessProperty =
        AvaloniaProperty.Register<SubProcItem, BuildSubprocess?>(nameof(MinHeight));

    public BuildSubprocess? Subprocess
    {
        get => GetValue(SubprocessProperty);
        set => SetValue(SubprocessProperty, value);
    }
    ...
    public void Update()
    {
        if (Subprocess == null)
            return;
        if (Subprocess.Compile)
        {
            _icon.Data = PathGeometry.Parse(
            ...
            _label.Text = "Компиляция";
        }
        else if (Subprocess.Command != null)
        {
            _icon.Data = PathGeometry.Parse(
            ...
            _label.Text = Subprocess.Command;
        }
